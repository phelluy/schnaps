...
bool submit_task() {
  ...
  for (int i = 0; i < size; ++i)
    test &= (fabs(buffer_in[i] - i) < _VERY_SMALL);
  assert(test);
  for (int i = 0; i < size; ++i) {
    if (!(fabs(buffer_out[i] - (schnaps_real) i * (1 + alpha)) < 1e5*_VERY_SMALL))
      printf("result[%d]: %f (should be %f) err=%.10e\n", i, buffer_out[i], i * (1 + alpha),
           fabs(buffer_out[i] - i * (1 + alpha)));
    test &= (fabs(buffer_out[i] - i * (1 + alpha)) < 1e5*_VERY_SMALL);
  }
