...
bool submit_task(Simulation* simu, schnaps_real* buffer) {
  ...
  for (int ifa = 0; ifa < simu->macromesh.nbfaces; ++ifa) {
    ...
    if (inter->vol_indexR) starpu_data_unregister(inter->vol_indexR_handle);
    starpu_data_unregister(inter->wL_handle);
    if (inter->wR) starpu_data_unregister(inter->wR_handle);
  ...
  for(int ie = 0; ie < simu->macromesh.nbelems; ++ie) {
    ...
    for (int i = 0; i < fsize; ++i) {
      if (!(fabs(buffer[ie * fsize + i] - f->res[i]) < _VERY_SMALL))
        printf("field: %d  reference[%d]: %f  result[%d]: %f eps=%f\n",
               ie, i, buffer[ie * fsize + i], i, f->res[i],
	       buffer[ie * fsize + i] - f->res[i]);
      test &= (fabs(buffer[ie * fsize + i] - f->res[i]) < _SMALL * 10);
    }
  }
...
#else
  sprintf(buf, " -cl-single-precision-constant -D schnaps_real=float -D _M=6");
#endif
